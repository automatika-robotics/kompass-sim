...
def generate_launch_description():
    ...
    webots_package_dir = get_package_share_directory("webots_ros2_tiago")
    mode = LaunchConfiguration("mode")
    use_sim_time = LaunchConfiguration("use_sim_time", default=False)
    run_rviz = LaunchConfiguration("run_rviz", default=True)

    webots = WebotsLauncher(
        world=PathJoinSubstitution([webots_package_dir, "worlds", "default.wbt"]),
        mode=mode,
    ...
    ros2_control_params = os.path.join(
        webots_package_dir, "resource", "ros2_control.yml"
    )
    ...
    robot_description_path = os.path.join(
        webots_package_dir, "resource", "tiago_webots.urdf"
    )
    ros2_control_params = os.path.join(
        webots_package_dir, "resource", "ros2_control.yml"
    )
